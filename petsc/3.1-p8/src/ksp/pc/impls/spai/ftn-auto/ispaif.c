...
extern "C" {
#endif
void PETSC_STDCALL   pcspaisetepsilon_(PC pc,double *epsilon1, int *__ierr ){
*__ierr = PCSPAISetEpsilon(
	(PC)PetscToPointer((pc) ),*epsilon1);
}
void PETSC_STDCALL   pcspaisetnbsteps_(PC pc,int *nbsteps1, int *__ierr ){
*__ierr = PCSPAISetNBSteps(
	(PC)PetscToPointer((pc) ),*nbsteps1);
}
void PETSC_STDCALL   pcspaisetmax_(PC pc,int *max1, int *__ierr ){
*__ierr = PCSPAISetMax(
	(PC)PetscToPointer((pc) ),*max1);
}
void PETSC_STDCALL   pcspaisetmaxnew_(PC pc,int *maxnew1, int *__ierr ){
*__ierr = PCSPAISetMaxNew(
	(PC)PetscToPointer((pc) ),*maxnew1);
}
void PETSC_STDCALL   pcspaisetblocksize_(PC pc,int *block_size1, int *__ierr ){
*__ierr = PCSPAISetBlockSize(
	(PC)PetscToPointer((pc) ),*block_size1);
}
void PETSC_STDCALL   pcspaisetcachesize_(PC pc,int *cache_size, int *__ierr ){
*__ierr = PCSPAISetCacheSize(
	(PC)PetscToPointer((pc) ),*cache_size);
}
void PETSC_STDCALL   pcspaisetverbose_(PC pc,int *verbose, int *__ierr ){
*__ierr = PCSPAISetVerbose(
	(PC)PetscToPointer((pc) ),*verbose);
}
void PETSC_STDCALL   pcspaisetsp_(PC pc,int *sp, int *__ierr ){
*__ierr = PCSPAISetSp(
